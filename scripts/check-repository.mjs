...
assert.equal(routes.routes["/meeting"], "meeting.html", "the shared Meeting Mode route must stay in the route contract");
for (const verb of ["start-meeting", "claim-meeting-processing", "add-meeting-note", "propose-meeting-action", "decide-meeting-action", "record-meeting-action-outcome", "end-meeting", "read-meeting"]) assert.ok(contract.mcp_operations.includes(verb), `the Meeting page needs ${verb} pinned`);
